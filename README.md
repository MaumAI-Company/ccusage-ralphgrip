...
ccusage-ralphgrip는 팀원 각자의 로컬 환경에 설치되는 **수집 플러그인**과,
...
│   └── installer/                 # npx ccusage-ralphgrip 설치기
...
```bash
curl -sL https://ccusage.ralphgrip.com/api/install | bash -s -- "홍길동"
```
...
```bash
git clone https://github.com/MaumAI-Company/ccusage-ralphgrip.git
...
현재 운영: AWS EC2 + PM2 + Nginx (`ccusage.ralphgrip.com`)
...
aws ssm send-command \
  ...
  --parameters '{"commands":["cd /opt/ccusage-ralphgrip && git pull origin main && pnpm install --frozen-lockfile && pnpm --filter dashboard build && HOME=/root pm2 reload ccusage-dashboard --update-env"]}' \
